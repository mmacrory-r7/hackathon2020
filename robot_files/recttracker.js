function RectTracker(undoStack) {
	...
	this.draw = function(context) {
		...
		for(var i=1; i < handles.length; i++) {
			...
			context.arc(handles[i].x,handles[i].y,this.handleRadius(),0,2*Math.PI);
			context.fill();
		}
	}
	...
	this.checkHandles = function(point) {
		for(var i=1; i<handles.length; i++) {
			if(Math.abs(handles[i].x - point.x) < this.handleRadius() && Math.abs(handles[i].y - point.y) < this.handleRadius())
				return i;
		}
	...
	this.redrawHandle = function(handleId){
		var handleRect = new Rect({x: handles[handleId].x, y: handles[handleId].y, width: 0, height: 0});
	...
	this.mouseMove = function(event) {
		...
		var cursors = ["move","nwse-resize","nesw-resize","nesw-resize","nwse-resize"];
		...
		if(activeHandle >=0 && startPoint) {
			...
			case 0:
				...
				break;
			case 1:
				this.rect.setTopLeft(point); break;
			case 2:
				this.rect.setTopRight(point); break;
			case 3:
				this.rect.setBottomLeft(point); break;
			case 4:
				this.rect.setBottomRight(point); break;
			}
	...
	this.mouseUp = function(event) {
		...
		undoStack.push( new ChangeLayerPropertyCommand(this.layer().layerId(), LayerProperties.boundingRect, this.layer().boundingRect(), this.rect));
