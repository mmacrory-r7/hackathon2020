...
var colors = {
	...
	"erase-matting": 'rgb(255,255,255)',
	"matting": 'rgb(255,255,0)'
};
...
ScribbleCommand.prototype.undo = function (model) {
	model.layer(this.layerId).popAction();
};
...
function ScribbleClearCommand(layer) {
	this.layer = layer;
	this.actions = layer.property(ScribbleProperties.actions);
};

ScribbleClearCommand.prototype.undo = function (model) {
	this.layer.setProperty(ScribbleProperties.actions, this.actions);
	this.layer.initActions();
	this.layer.drawLayer();
};

ScribbleClearCommand.prototype.redo = function (model) {
	this.layer.setProperty(ScribbleProperties.actions, []);
	this.layer.drawLayer();
};
...
MarkerAction.prototype.compressPoints = function() {
	return;
	var linePoints = [];
	...
	for(var i = 1; i < this.points.length; i++) {
		...
		var lastId = linePoints.length - 1;
		if(pt[0] == linePoints[lastId][0] || pt[1] == linePoints[lastId][1]){
			continue;
		} else {
			linePoints.push(pt);
		}
...
ScribbleLayer.prototype.initActions = function(){
	...
	actions.forEach(function(action){
		if(action.type == "marker"){
			var markerAct = new MarkerAction(action.color, action.lineWidth, action.points);
			markerAct.compressPoints();
			newActions.push(markerAct);
		}
	},this);
...
ScribbleLayer.prototype.init = function(data){
	Layer.prototype.init.call(this, data);
	this.initActions();
}
...
ScribbleLayer.prototype.popAction = function() {
	var actions = this._properties[ScribbleProperties.actions];
	actions.pop();
	this.drawLayer();
	this.save();
